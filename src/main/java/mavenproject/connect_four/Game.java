...
public class Game {
	...
	public void setUpGame() throws InvalidBoardSizeException {
		...
		System.out.print("Enter the number of players involved: ");
		...
		for (int i = 0; i < numOfPlayers; i++) {
			...
			if (players.isEmpty()) {
				...
				if (players.contains(name)) {
					...
					name = scanner.nextLine();
					while (players.get(counter).getName().equalsIgnoreCase(name)) {
						System.out.println("'" + name + "' already exists!");
						System.out.println("Please enter a different name");
						name = scanner.nextLine();
					}
	...
	public void play() {
		...
		while (inaRow < 4 || inaRow > 6) {
			System.out.println("Invalid Input.\nPlease Choose the correct option");
			inaRow = scanner.nextInt();
		}

		System.out.println("You have 5 minutes to complete this game.");
		// Get the starting time
		long startTime = System.currentTimeMillis();
		long endTime = System.currentTimeMillis();
		// Check if there is no winner and the time is less than 5 minutes, if true it
		// will keep running if false it will print the winner and terminate the
		// program.
		while (noWinner && (endTime - startTime <= 300000)) {
			...
			// Call playerTurn() method of Game class and handle any exception it may
			// generate
			try {
				playerTurn(currentPlayer);
			} catch (InvalidMoveException | ColumnFullException e) {
				System.out.println(e.getMessage());
				// reassign the variable to allow the game to continue. Note the index would
				// wrap back to the first player if we are at the end. Think of using modules
				// (%)
			...
			if (board.checkIfPlayerIsTheWinner(currentPlayer.getPlayerNumber(), inaRow)) {
				...
				currentPlayerIndex = (currentPlayerIndex == players.size() - 1) ? 0 : currentPlayerIndex + 1;
				; // reassign the variable to allow the game to continue. Note the index would
					// wrap back to the first player if we are at the end. Think of using modulus
					// (%).
			}
